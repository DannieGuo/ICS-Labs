...
void eval(char *cmdline) 
{
	...
	int bgfg =  parseline(cmdline, argv); /*bg 1 fg 0*/
...
int builtin_cmd(char **argv) 
{
	...
	if(!strcmp(argv[0], "jobs"))
	{
		sigset_t mask, prev;
    	sigfillset(&mask);
		sigprocmask(SIG_BLOCK, &mask, &prev); /*prevent interrupted*/
		listjobs(jobs);
...
{ 
	...
	if(id[0] == '%') /*jid*/
	{
		...
		if(job == NULL) /*can not find this job*/
		{
	...
	else
	{
		kill(-pid, SIGCONT);
		job->state = FG;
		waitfg(pid); /*wait for the foreground job finish*/ 
	}
	sigprocmask(SIG_SETMASK, &prev, NULL); /*unblock*/
    return;
}
...
void waitfg(pid_t pid)
{
	sigset_t mask;
    sigemptyset(&mask);
	while(fgpid(jobs) != 0) /*if there is a fg job*/
	{
		sigsuspend(&mask); /*unblock and pause*/
	}
	return;
}
 ...
 *     the user types ctrl-z at the keyboard. Catch it and 
 		suspend the
